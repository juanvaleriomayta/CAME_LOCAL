...
CASE
    WHEN PAGOS.ASUMPAG = 'E' THEN 'Empresa'
    WHEN PAGOS.ASUMPAG = 'P' THEN 'Participante'
END AS ASUMEPAG,
CASE
    WHEN PAGOS.ESTPAG = 'N' THEN 'Normal'
    WHEN PAGOS.ESTPAG = 'D' THEN 'Descuento'
END AS ESTADOPAG,
PAGOS.CONDPAG AS "CONDIPAG",
PAGOS.OBSEPAG AS "OBSERPAG",
PAGOS.ENCPAG AS "ENCAPAG",
PAGOS.EMAILENC AS "ENCEMAIL",
PAGOS.TELFENC AS "ENCTELF",
TO_CHAR(PAGOS.FECHAREG ,'DD/MM/YYYY') AS "FECHAREGA",
ENTREVISTA.PRE1 AS "PREGUNTA1",
ENTREVISTA.PRE2 AS "PREGUNTA2",
ENTREVISTA.COMENT AS "COMENTARIOS",
ENTREVISTA.ESTENT AS "ESTADO",
TO_CHAR(ENTREVISTA.FECENT,'DD/MM/YYYY') AS "FECHAREGE"
FROM PERSONA INNER JOIN PERSONA_DET ON PERSONA.CODPER = PERSONA_DET.CODPER
INNER JOIN EMPRESA ON PERSONA.CODEMP = EMPRESA.CODEMP INNER JOIN UBIGEO ON 
PERSONA.CODUBI = UBIGEO.CODUBI INNER JOIN ASIGNACION ON ASIGNACION.CODPER = PERSONA.CODPER
INNER JOIN PAGOS ON PERSONA.CODPER = PAGOS.CODPER INNER JOIN ENTREVISTA ON
PERSONA.CODPER = ENTREVISTA.CODPER;